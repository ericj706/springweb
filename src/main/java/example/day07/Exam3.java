...
public class Exam3 {
    public static void main(String[] args) {
        ...
        // 2. 스트림 API, 리스트객체.stream().중간연산1().중간연산2().최종연산();
            // 1) stream().forEach( (순회변수명)->{싱행문}); , return 없는 단순 순회제공
            numbers.stream().forEach((data)->{System.out.println(data);});
            // 2) stream().map( (순회변수명)->{실행문;}).toList();
            ...
            // 4) sorted(): 오름차순 정렬 / sorted(Comparator.reverseOrder()): 내림차순
