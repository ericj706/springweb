...
public interface TodoRepository extends JpaRepository<TodoEntity,Integer>{
    ...
    // 1. CRUD메소드 제공: .save() .findAll() .findById() .deleteById()
    // 2. 쿼리메소드 : SQL 작성하지 않고 추상메소드 이름으로 쿼리 자동 생성 <카멜표기법>
    // 반환타입 = findBy필드명(타입 매개변수명); / 주의할점: 필드명에 대소문자(카멜)규칙
    TodoEntity findByTitle (String title);
    TodoEntity findByTitleAndContent(String title, String content);
    Map<String, Object> findByTitleOrContetn(String title, String content);

    // 2. 네이티브쿼리: SQL 직접 작성, 추상메소드 이름은 아무거나
    // SQL문법내 매개변수 대입시 ? 대신에 : 매개변수명
